...
const user1 = users.find(u => u.id == comments.find(c => c.text == 'OK great thanks').userId);
console.log(user1)
...
const comm = comments.map(comment => {
    const { firstName, lastName } =
        users.find(usee => usee.id === comment.userId)
    return { ...comment, firstName, lastName }
})
console.log(comm)

// 5) Get a list of the users who haven't commented
const usersWithoutComments = users.filter(tuser => !comments.
    find(comment => comment.userId === tuser.id))
console.log(usersWithoutComments)